...
let fetchdata = async () => {
  const response = await fetch(
    "https://newsapi.org/v2/top-headlines?country=in&apiKey=fbf86f4db86a4a44a80560814dbdc318&category=science&pageSize=10&page=1"
  );
...
window.addEventListener("scroll", () => {
  ...
  if (
    ...
    setTimeout(() => {
      fetch(
        `https://newsapi.org/v2/top-headlines?country=in&apiKey=fbf86f4db86a4a44a80560814dbdc318&category=science&pageSize=10&page=${page}`
      )
        .then((res) => res.json())
        .then((json) => {
          json.articles.map((ele) => {
